...
import SkodaAuthenticator from "#lib/api/authenticators/skoda-authenticator.mjs";
import SkodaUser from "#lib/api/users/skoda-user.mjs";
	...
	public async onPair(session: PairSession): Promise<void> {
		...
		session.setHandler(
			...
			async (data: {
				...
				user = new SkodaUser(authenticator);
		...
		session.setHandler(
			...
			async (pincode: string[]): Promise<boolean> => {
				...
				const userInstance = user ?? new SkodaUser(authenticator);
