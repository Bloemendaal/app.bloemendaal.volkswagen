...
export default class Processor implements Processable {
  ...
  public async register(fetchData: FetchData): Promise<void> {
    ...
    for (const processable of this.processables) {
      try {
        const name = processable.constructor.name;
        console.log(`[Processor] Registering: ${name}`);
        await processable.register(fetchData);
      } catch (error) {
        const name = processable.constructor.name;
        console.error(`[Processor] Error registering ${name}:`, error);
        errors.push(error);
      }
    }
  ...
  public async run(fetchData: FetchData, options?: RunOptions): Promise<void> {
    ...
    for (const processable of this.processables) {
      try {
        const name = processable.constructor.name;
        console.log(`[Processor] Running: ${name}`);
        await processable.run(fetchData, options);
      } catch (error) {
        const name = processable.constructor.name;
        console.error(`[Processor] Error running ${name}:`, error);
        errors.push(error);
      }
    }

    if (errors.length) {
      console.error(`[Processor] Total errors: ${errors.length}`);
      throw new AggregateError(
        errors,
